...
class FabTaskCommand(GitWindowCommand):
    def run(self):
        self.run_command(['fab', '--fabfile=/root/workspace/newgit/manager/fabfile.py', '--list', '--short'], self.branch_done)
    ...
    def panel_done(self, picked):
        if 0 > picked < len(self.results):
            return
        picked_task = self.results[picked].strip()
        task = '%s:role=test' % picked_task
        self.run_command(['fab', '--fabfile=/root/workspace/newgit/manager/fabfile.py', task])
